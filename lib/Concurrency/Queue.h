...
//                      Juan Fco Rascón Crespo <jfrascon@gmail.com>
//
...
class Queue
{
 ...
 private:
  // decided to go with a list so we can trust the iterator. With other
  // containers you can have
  // reallocation that invalidates iterator
  std::list<T> queue_;
  typedef typename std::list<T>::iterator QueueIterator;

  // could be a single map with a structure. But keep in this way cause it helps
  // in function unsubscribe
